...
const FormReplaceOil = ({ carId }) => {
    ...
    useEffect(() => {
        const getData = async () => {
            let [replaceRs, carRs] = await Promise.all([
                makeRequest({
                    method: "GET",
                    url: requestUrl.replaceOil.readUrl(),
                }),
                makeRequest({
                    method: "GET",
            ...
            setReplace(addListKey(replaceRs.data));
            setCars(addListKey(carRs.data));
            setPagination({ ...pagination, total: replaceRs.totalRecords });
            setLoading(false);
        };
        getData();
    }, []);
    ...
    const handleSearch = async (params, pagination = {}) => {
        ...
        makeRequest({
            ...
            setReplace(addListKey(searchRs.data));
            setPagination({
                ...pagination,
    ...
    const onFinish = (replace) => {
        let date = replace.date ? replace.date.format("YYYY-MM-DD") : null;
        let data = { ...replace, date };
        ...
        let url =
            mode === RECORD_MODE.CREATE
                ? requestUrl.replaceOil.createUrl()
                : requestUrl.replaceOil.updateUrl({ id: replace.id });
        makeRequest({
            method,
            url,
            data,
        ...
        <>
            ...
            >
                ...
                >
                    ...
                    <Form.Item label="Nội dung thay dầu" name="content">
